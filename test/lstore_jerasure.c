/*
 * lstore-jerasure - Command line tool using the Jerasure (JE) erasure backend
 *
 * Build: make lstore-jerasure
 *
 * Encode a file (produces parity file, data file is left as-is or you can stripe it yourself):
 *   ./lstore-jerasure encode <input> <parity> <k> <m> [w=8] [method=2] [strip_size=0(auto)] [packet=-1] [base=-1]
 *
 * Decode / recover:
 *   ./lstore-jerasure decode <erased_data> <parity> <recovered> <k> <m> <w> <packet> <base> <strip_size> <n_missing> <idx>...
 *
 * Example (k=6 m=3 Cauchy):
 *   ./lstore-jerasure encode myfile.bin myfile.parity 6 3
 *   # later, after some erasures:
 *   cp myfile.bin erased.bin
 *   ./lstore-jerasure decode erased.bin myfile.parity recovered.bin 6 3 8 -1 -1 0 1 2 8
 */
...
#define DEFAULT_METHOD  2   /* CAUCHY_ORIG */
...
static void usage(const char *prog) {
    fprintf(stderr,
        "Usage:\n"
        "  %s [--default] encode <input> <parity> <k> <m> [w=8] [method=2] [strip=16k] [packet=-1] [base=-1]\n"
        ...
        "method: 2=CAUCHY_ORIG (best for cross-compat tests)\n"
...
static lio_erasure_plan_t *default_plan(long long fsize) {
    long long strip;
    int packet;
    int base = DEFAULT_BASE;
    default_layout(fsize, DEFAULT_K, DEFAULT_W, base, &strip, &packet);
    return et_new_plan(DEFAULT_METHOD, strip, DEFAULT_K, DEFAULT_M,
...
static int wipe_blocks(lio_erasure_plan_t *plan,
...
static int build_default_erasures(lio_erasure_plan_t *plan, int **erasures_out) {
...
int main(int argc, char **argv) {
    ...
    if (strcmp(cmd, "encode") == 0) {
        ...
        int method = use_default ? DEFAULT_METHOD : (argc > arg_start + 6 ? atoi(argv[arg_start + 6]) : 2);
        ...
        if (use_default) {
            ...
            plan = et_generate_plan(hint, method, k, m, w, packet, base);
        }
        if (!plan) {
            fprintf(stderr, "Failed to create plan (Jerasure)\n");
            return 1;
        }

        printf("lstore-jerasure encode (Jerasure backend)\n");
        printf("  input=%s k=%d m=%d w=%d method=%d strip=%lld packet=%d base=%d\n",
               input, k, m, w, method, plan->strip_size, plan->packet_size, plan->base_unit);

        int rc = et_encode(plan, input, 0, parity, 0, 0);
        ...
        if (use_default) {
            ...
            lio_erasure_plan_t *plan = default_plan(fsize);
            if (!plan) {
                fprintf(stderr, "Failed to create plan (Jerasure)\n");
                return 1;
            }

            printf("lstore-jerasure decode --default (Jerasure backend)\n");
            printf("  k=%d m=%d w=%d strip=%lld packet=%d base=%d\n",
                   plan->data_strips, plan->parity_strips, plan->w,
                   plan->strip_size, plan->packet_size, plan->base_unit);
            ...
            if (wipe_blocks(plan, DEFAULT_ERASED, 0, erased_parity, 0,
            ...
            if (build_default_erasures(plan, &erasures) != 0) {
                et_destroy_plan(plan);
                return 1;
            }
            ...
            int rc = et_decode(plan, fsize, DEFAULT_RECOVERED, 0,
            ...
            if (rc == 0) {
                ...
                fprintf(stderr, "et_decode returned error %d\n", rc);
            }
            return rc;
        }
        ...
        lio_erasure_plan_t *plan = et_new_plan(2 /*CAUCHY*/, strip, k, m, w, packet, base);
        if (!plan) {
            fprintf(stderr, "Failed to create plan (Jerasure)\n");
            free(erasures);
            return 1;
        ...
        printf("lstore-jerasure decode (Jerasure backend)\n");
        printf("  Writing recovered data to %s (in-place update on the copy)\n", output);

        int rc = et_decode(plan, fsize, output, 0, parity, 0, 0, erasures);
