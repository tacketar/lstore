...
#else
                    __xstat_stdio(0, fold, &sbuf);
#endif
                    err = lio_fs_object_create(fs_new, NULL, fnew, sbuf.st_mode, 0);
                } else {  //** Destination already exists
                    if ((ftype & OS_OBJECT_DIR_FLAG) == 0) {
                        errno = ENOTDIR;
                        err = -1;
...
struct dirent64 *WRAPPER_PREFIX(readdir64)(DIR *dirp)
{
    ...
    err = lio_fs_readdir(cfd->ldir, &dname, &stat, NULL, NULL);
    ...
    while (i<argc) {
        if (strcmp(argv[i], "--fs") == 0) { //** Got a match
            ...
            if (argv[i][0] == '/') { //** Missing the config so use the global
                ...
                user = section = NULL;
                if (lio_parse_path(argv[i], &user, NULL, NULL, NULL, NULL, &section, NULL, NULL, 1) == -1) {
