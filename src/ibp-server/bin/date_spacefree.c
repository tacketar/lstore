...
void parse_line(char *buffer, apr_time_t *t, uint64_t *a_count, uint64_t *p_count,
                uint64_t *bytes_used, uint64_t *bytes)
{
    char *bstate;
    int fin;
    *t = 0;
...
int main(int argc, char **argv)
{
    ...
    if (print_full) {
        printf
        ("Time date Tallocs Talias Ttotal mb_used mb_max Nallocs Nalias Ntotal total_used_mb  total_max_mb\n");
    } else {
...
//printf("err=%d buf=%s\n", err, buffer);
        if (err == NS_OK) {
            if (strcmp("END", buffer) == 0) {   //** Finished
                ...
                a_total = a_total + a_count;
                p_total = p_total + p_count;
                ...
                if (print_full) {
                    printf(TT " * %s " LU " * " LU " * " LU " * %lf * %lf * " LU " * " LU " * " LU
                           " * %lf * %lf\n", t, print_time, a_count, p_count, count, fb1, fb2,
                           a_total, p_total, total, fb3, fb4);
                } else {
