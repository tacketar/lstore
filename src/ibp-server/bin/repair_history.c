...
//printf("sizeof(Allocation_t)=" LU " ALLOC_HEADER=%d\n", sizeof(Allocation_t), ALLOC_HEADER);
    ...
    changed = 0;
    if ((h.manage_slot < 0) || (h.manage_slot >= ALLOC_HISTORY)) {
        changed = 1;
        h.manage_slot = 0;
    }
    if ((h.read_slot < 0) || (h.read_slot >= ALLOC_HISTORY)) {
        changed = 1;
        h.read_slot = 0;
    }
    if ((h.write_slot < 0) || (h.write_slot >= ALLOC_HISTORY)) {
        changed = 1;
        h.write_slot = 0;
    }
