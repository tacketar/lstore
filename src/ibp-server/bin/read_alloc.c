...
//printf("sizeof(Allocation_t)=" LU " ALLOC_HEADER=%d\n", sizeof(Allocation_t), ALLOC_HEADER);
    ...
    if (argc < 2) {
        ...
        printf
        ("where --file stores a portion of the allocation to fname based on the given offset and length\n");
        printf
        ("          fname of where to store data.  If stdout or stderr redirects to that device\n");
        printf
...
//nblocks = 1;
        ...
        for (i = 0; i < n; i++) {
            ...
            osd_get_chksum(dev, 0, block_chksum, calc_chksum, sizeof(block_chksum), blen,
                           good_block, start_block, end_block);
            k = end_block - start_block + 1;
            for (j = 0; j < k; j++) {
                b = start_block + j;
                bytes_used = blen[j];
                s = good_block[j];
                tbx_chksum_bin2hex(bin_len, (unsigned char *) &(block_chksum[j * bin_len]),
                                   hex_digest);
                if (s == 0) {
                    printf("%7d  %10lld   %2d     %s\n", b, bytes_used, s, hex_digest);
                } else {
                    tbx_chksum_bin2hex(bin_len, (unsigned char *) &(calc_chksum[j * bin_len]),
                                       hex_digest2);
