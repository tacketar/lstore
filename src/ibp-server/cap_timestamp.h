...
void set_rw_ts(Allocation_rw_ts_t *ts, int *slot, Allocation_address_t *add, uint64_t offset,
               uint64_t size, osd_id_t id);
void set_manage_ts(Allocation_manage_ts_t *ts, int *slot, Allocation_address_t *add, int cmd,
                   int subcmd, int reliability, uint32_t expiration, uint64_t size, osd_id_t pid);
