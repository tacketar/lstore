...
void iovec_start(ibp_iovec_t *iovec, int *index, ibp_off_t *ioff, ibp_off_t *ileft)
{
    ...
    for (i = 0; i < iovec->n; i++) {
        log_printf(15,
                   "iovec_start: i=%d transfer_total=" I64T " off=" I64T " len=" I64T " cumlen= "
                   I64T "\n", i, iovec->transfer_total, iovec->vec[i].off, iovec->vec[i].len,
                   iovec->vec[i].cumulative_len);
        if (iovec->transfer_total < iovec->vec[i].cumulative_len)
            break;
    }
    ...
    if (i < iovec->n) {
        *index = i;
        ...
        if (i > 0)
            n = iovec->transfer_total - iovec->vec[i - 1].cumulative_len;
    ...
    log_printf(15,
               "iovec_start: n=%d transfer_total=" I64T " index= %d ioff=" I64T " ileft=" I64T
...
int disk_to_disk_copy_user(Resource_t *src_res, osd_id_t src_id, ibp_off_t src_offset,
                           Resource_t *dest_res, osd_id_t dest_id, ibp_off_t dest_offset,
                           ibp_off_t len, apr_time_t end_time)
{
    int i;
    osd_fd_t *sfd, *dfd;
    ...
    log_printf(15,
               "disk_to_disk_copy_user: src_id=" LU " src_offset=" I64T " dest_id=" LU
    ...
    if (sfd == NULL) {
        log_printf(0,
                   "disk_to_disk_copy_user: Error with src open_allocation(-res-, " LU ", " I64T
    ...
    if (dfd == NULL) {
        err = errno;
        log_printf(0,
                   "disk_to_disk_copy_user: Error with dest open_allocation(-res-, " LU ", " I64T
    ...
    for (i = 0; i < len; i = i + bufsize) {
        ...
        if (err != 0) {
            log_printf(0,
                       "disk_to_disk_copy_user: Error with read_allocation(-res-, " LU ", " I64T
        ...
        if (err != 0) {
            log_printf(0,
                       "disk_to_disk_copy_user: Error with write_allocation(-res-, " LU ", " I64T
...
int read_from_disk_user(ibp_task_t *task, Allocation_t *a, ibp_off_t *left, Resource_t *res)
{
    ...
    nbytes = a->size;
    log_printf(10,
               "read_from_disk: ns=%d id=" LU " a.size=" I64T " a.r_pos=" I64T " len=" I64T "\n",
    ...
    do {
        ...
        do {
            ...
            if (err != 0) {
                char tmp[128];
                log_printf(0,
                           "read_disk: Error with read_allocation(%s, " LU ", " I64T ", " I64T
                           ", buffer) = " I64T "\n", ibp_rid2str(res->rid, tmp), a->id, ioff,
                           cleft, err);
                shortwrite = 100;
                nwrite = err;
            }
        ...
        do {                    //** Loop until data is completely sent or blocked
            ...
            log_printf(15,
                       "read_from_disk: id=" LU " -- bpos=" I64T " bleft=" I64T ", ntotal=" I64T
                       ", nwrite=" I64T " * shortwrite=" I64T " ns=%d\n", a->id, bpos, bleft,
                       btotal, nwrite, shortwrite, tbx_ns_getid(task->ns));
        ...
        *left -= btotal;
        ...
        log_printf(15,
                   "read_from_disk: nleft=" I64T " nwrite=" I64T " off=" I64T " shortwrite=" I64T
    ...
    if ((nwrite < 0) || (shortwrite >= 100)) {  //** Dead connection
        ...
        if (*left == 0) {       //** Finished data transfer
            ...
            log_printf(10,
                       "read_from_disk: returning ns=%d back to caller.  short read.  tid=" LU
                       "\n", tbx_ns_getid(task->ns), task->tid);
            task_status = 0;
        }
    }
...
int write_to_disk_user(ibp_task_t *task, Allocation_t *a, ibp_off_t *left, Resource_t *res)
{
    ...
    if (a->type == IBP_BYTEARRAY) {
        ...
        nleft =
            (*left >
             ((ibp_off_t) a->max_size - (ibp_off_t) a->size)) ? ((ibp_off_t) a->max_size -
                     (ibp_off_t) a->size) : *left;
    }

    ntotal = 0;
    debug_printf(10,
                 "write_to_disk_user(BA): start.... id=" LU " * max_size=" I64T " * curr_size="
                 I64T " * max_transfer=" I64T " left=" I64T " ns=%d\n", a->id, a->max_size,
                 a->size, nleft, *left, tbx_ns_getid(task->ns));
    ...
    do {
        ...
        do {
            ...
            log_printf(10,
                       "write_to_disk_user: id=" LU " ns=%d inner loop ncurrread= " I64T " bpos="
                       I64T " nbytes=" I64T " shortread=%d bufsize=" ST "\n", a->id,
                       tbx_ns_getid(task->ns), ncurrread, bpos, nbytes, shortread, sizeof(buffer));
        } while ((nbytes > 0) && (shortread < 3));
        nread = bpos;

        log_printf(10,
                   "write_to_disk_user: id=" LU " ns=%d after loop ncurrread= " I64T " bpos=" I64T
        ...
        if (nread > 0) {
            ...
            do {
                ...
                if (err != 0) {
                    char tmp[128];
                    log_printf(0,
                               "write_to_disk_user: Error with write_allocation(%s, " LU ", " I64T
                               ", " I64T ", buffer) = " I64T "  tid=" LU "\n",
        ...
        log_printf(15,
                   "write_to_disk_user: id=" LU " left=" I64T " -- pos=" I64T ", nleft=" I64T
                   ", ntotal=" I64T ", nread=" I64T " ns=%d shortread=%d\n", a->id, *left, ioff,
                   nleft, ntotal, nread, tbx_ns_getid(task->ns), shortread);
...
//  } while ((ntotal < nleft) && (shortread < 3));
    ...
    *left = nleft;
    ...
    if (shortread >= 100) {     //** Dead connection
        ...
        if (*left == 0) {       //** Finished data transfer
            log_printf(10,
                       "write_to_disk_user: Completed transfer! ns=%d tid=" LU " a.size=" I64T
                       "\n", tbx_ns_getid(task->ns), task->tid, a->size);
            task_status = 1;
        } else {
            log_printf(10,
                       "write_to_disk_user: task_status=%d returning ns=%d back to caller.  a.size="
                       LU " short read.  tid=" LU "\n", task_status, tbx_ns_getid(task->ns),
                       a->size, task->tid);
        }
    }
...
int disk_to_disk_copy(Resource_t *src_res, osd_id_t src_id, ibp_off_t src_offset,
                      ...
                      ibp_off_t len, apr_time_t end_time)
{
    return (disk_to_disk_copy_user
            (src_res, src_id, src_offset, dest_res, dest_id, dest_offset, len, end_time));
}
