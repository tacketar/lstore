...
void add_stat(Transfer_stat_t *s)
{
    ...
    if (s->dir == DIR_IN) {
        ...
        if (s->dir == DEPOT_COPY_OUT)
            stats.copy_bytes += s->nbytes;
    }
...
int send_stats(tbx_ns_t *ns, ibp_time_t start_time, tbx_ns_timeout_t dt)
{
    ...
    n = tbx_append_printf(sptr, &nchar, bufsize, "%d\n", currtime);

    for (i = 0; i < global_config->server.max_threads; i++) {
        ts = global_task[i].stat;
        if (ts.start > 0) {
            if (ts.dir == DIR_IN) {
                tbx_append_printf(sptr, &nchar, bufsize,"%s %s %d %d %d %d 0\n", ts.address,
                                  stats.host, ts.start, currtime, ts.nbytes, ts.id);
            } else {
                tbx_append_printf(sptr, &nchar, bufsize, "%s %s %d %d %d %d 0\n", stats.host,
                             ts.address, ts.start, currtime, ts.nbytes, ts.id);
            }
        }
    }
    ...
    log_printf(15, "send_stats: ns=%d start=%lu s[i].start=%d\n", tbx_ns_getid(ns), start_time,
               s[i].start);
    while ((s[i].start > start_time) && (j != stats.size)) {
        if (ts.dir == DIR_IN) {
            tbx_append_printf(sptr, &nchar, bufsize, "%s %s %d %d %d %d 1\n", s[i].address,
                         stats.host, s[i].start, s[i].end, s[i].nbytes, s[i].id);
        } else {
            tbx_append_printf(sptr, &nchar, bufsize, "%s %s %d %d %d %d 1\n", stats.host,
                         s[i].address, s[i].start, s[i].end, s[i].nbytes, s[i].id);
        }
...
//log_printf(15, "send_stats: internal ns=%d sending: %s\n", tbx_ns_getid(ns), buffer);
    ...
    tbx_append_printf(sptr, &nchar, bufsize, "\n");

    log_printf(15, "send_stats: ns=%d nchar=%d strlen=" ST " sending: %s\n", tbx_ns_getid(ns),
               nchar, strlen(buffer), buffer);
