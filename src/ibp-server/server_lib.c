...
tbx_ns_timeout_t *convert_epoch_time2net(tbx_ns_timeout_t *tm, apr_time_t epoch_time)
{
    ...
    if (apr_time_sec(dt) < 0)
        dt = 5;                 //** Even if it's timed out give it a little time
...
//   apr_thread_mutex_unlock(task->lock);
    ...
    do {
        ...
        if ((request_task_close() == 1) && (offset <= 0))
            close_request = 1;
        log_printf(15,
                   "read_command: ns=%d nbytes=%d offset=%d status=%d sizeof(buffer)=" LU
                   " close_req=%d\n", tbx_ns_getid(ns), nbytes, offset, status, sizeof(buffer),
                   close_request);
    } while ((offset < (count - 1)) && (status == 0) && (apr_time_now() <= endtime)
             && (close_request == 0));
    nbytes = offset;
...
int handle_task(ibp_task_t *task)
{
    ...
    if ((cmd->command < 0) || (cmd->command > COMMAND_TABLE_MAX)) {
        ...
        mycmd = &(global_config->command[cmd->command]);
        if (mycmd->execute != NULL)
            err = mycmd->execute(task);
    }

    log_printf(10, "handle_task: tid=" LU " After handle_command " TT " ns=%d\n", task->tid,
               apr_time_now(), tbx_ns_getid(task->ns));


    tt = apr_time_now();
    log_printf(10,
               "handle_task: ns=%d err=%d ***END*** tid=" LU " Completed processing at " TT "\n",
...
//server_ns_readline_raw(task.ns, buffer, sizeof(buffer), dt, &status);
    ...
    while ((shutdown_request() == 0) && (closed == 0)) {
        ...
        if (status == 0) {
            ...
            log_printf(10,
                       "worker_task: ns=%d myid=%d command=%d start_time=" TT " end_time=" TT
                       " dt_read=" TT " dt_handle=" TT " dt_total=" TT "\n", tbx_ns_getid(th->ns),
                       myid, task.cmd.command, start_read, end_time, dt_read, dt_handle, dt_total);
            start_read = apr_time_now();
        } else if (status == -1) {
            closed = 1;
        }
...
//      if ((apr_time_now() - start_read) > global_config->server.min_idle) closed = 1;
        if ((apr_time_now() - start_read) > global_config->server.min_idle) {
            ...
            log_printf(10,
                       "worker_task: ns=%d myid=%d MIN_IDLE=" TT " exiting at " TT " start_read="
                       TT " dt=" TT "sec\n", tbx_ns_getid(th->ns), myid,
...
void reject_close(tbx_ns_t *ns)
{
    ...
    log_printf(15,
               "reject_close: ns=%d  curr_threads=%d request_thread=%d reject_count=%d dt=%lf\n",
               tbx_ns_getid(ns), taskmgr.curr_threads, taskmgr.request_thread,
               taskmgr.reject_count, dt);
    apr_thread_mutex_unlock(taskmgr.lock);
