...
#define FS_OBJ_ENCODING_COUNT 10000
...
osd_off_t fs_normal_read(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t offset, osd_off_t len,
                         buffer_t buffer);
osd_off_t fs_chksum_read(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t block, osd_off_t len,
                         buffer_t buffer);
osd_off_t fs_normal_write(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t offset, osd_off_t len,
                          buffer_t buffer);
osd_off_t fs_chksum_write(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t block, osd_off_t len,
                          buffer_t buffer);
int _fs_calculate_chksum(osd_fs_t *fs, osd_fs_fd_t *fsfd, tbx_chksum_t *cs, osd_off_t block,
                         char *buffer, osd_off_t bufsize, char *calc_chksum, int correct_errors);
osd_off_t _chksum_buffered_read(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t len, char *buffer,
                                osd_off_t bufsize, tbx_chksum_t *cs1, tbx_chksum_t *cs2);
...
fs_cache_table_t *fs_cache_create(apr_pool_t *pool, int n_blocks, int block_size,
                                  apr_time_t max_wait)
{
    int i;
    fs_cache_table_t *c;
    ...
    for (i = 0; i < n_blocks; i++)
        c->block_free_count[i] = block_size;
...
char *trashid2fname_normal(osd_fs_t *fs, int trash_type, const char *trash_id, char *fname,
                           int len)
{
    switch (trash_type) {
    case OSD_EXPIRE_ID:
...
char *trashid2fname_loopback(osd_fs_t *fs, int trash_type, const char *trash_id, char *fname,
                             int len)
{
    return (NULL);
}
               ...
               osd_off_t *start_size, osd_off_t *end_size)
{
    ...
    if (offset < fcs->header_blocksize) {
        ...
        *start_offset = offset;
        *start_size =
            ((len + *start_offset) > fcs->header_blocksize) ? fcs->header_blocksize - offset : len;
    } else {
        n = offset - fcs->header_blocksize;
        *start_block = (n / fcs->blocksize) + 1;
        *start_offset = n % fcs->blocksize;
        *start_size =
            ((len + *start_offset) > fcs->blocksize) ? fcs->blocksize - *start_offset : len;
    }
...
int insert_range(tbx_pc_t *coop, osd_fs_fd_t *fsfd, int rtype, int64_t timestamp,
...
int check_range_overlap(tbx_pc_t *coop, int64_t timestamp, osd_off_t start_block,
                        osd_off_t *end_block)
{
    tbx_pc_iter_t it;
    tbx_pch_t i;
    ...
    while ((r = tbx_pch_data(&i)) != NULL) {
        if ((r->lo <= start_block) && (r->hi >= start_block)) { //** Range overlaps beginning of block so may need to return
            if ((r->type == RANGE_INUSE)
                || ((r->type == RANGE_REQUEST) && (r->timestamp < timestamp))) {
                log_printf(10,
                           "check_range_overlap: my_ts=" I64T " my_lo=" I64T " my_hi=" I64T
                           " CONFLICT ts=" I64T " type=%d lo=" I64T " hi=" I64T "\n", timestamp,
                           start_block, *end_block, r->timestamp, r->type, r->lo, r->hi);
                return (1);
            }
            ...
            if ((r->type == RANGE_INUSE)
                ...
                log_printf(10,
                           "check_range_overlap: my_ts=" I64T " my_lo=" I64T " my_hi=" I64T
                           " SHRINK ts=" I64T " type=%d lo=" I64T " hi=" I64T "\n", timestamp,
                           start_block, *end_block, r->timestamp, r->type, r->lo, r->hi);
                *end_block = r->lo - 1;
            }
        }
...
osd_off_t fsfd_lock(osd_fs_t *fs, osd_fs_fd_t *fsfd, int mode, osd_off_t start_block,
                    osd_off_t end_block, osd_off_t *got_block)
{
    int err;
    int64_t timestamp;
    osd_fs_object_t *obj = fsfd->obj;
...
int _fs_read_block_header(osd_fs_fd_t *fsfd, uint32_t *block_bytes_used, char *cs_value,
                          osd_off_t max_blocksize, int cs_len)
{
    osd_off_t n, m;
    int err;
    ...
    if (m != n)
        err = -1;
    if (*block_bytes_used > max_blocksize) {
        *block_bytes_used = max_blocksize;
        err = -2;
    }
...
int _fs_write_block_header(osd_fs_fd_t *fsfd, uint32_t block_bytes_used, char *cs_value,
                           int cs_len)
{
    osd_off_t n, m;
...
osd_id_t fs_trash_undelete(osd_t *d, int trash_type, const char *trash_id)
{
    ...
    log_printf(15,
               "trash_undelete: ttype=%d trash_id=%s tname=%s dname=%s id=" LU " rename()=%d\n",
...
osd_off_t fs_offset_p2l(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t p_offset)
{
    ...
    if (n < fcs->hbs_with_chksum) {
        ...
        if (rem < 0) {
            log_printf(0,
                       "fs_offset_p2l: rem<cs_size! p_offset=" I64T " l_offset=" I64T " rem=" I64T
                       " hds=" I64T "\n", p_offset, n, rem, hds);
            n = n + rem;
        }
...
int fs_truncate(osd_t *d, osd_id_t id, osd_off_t l_size)
{
    ...
    if (l_size > 0)
        l_size--;               //** This is the final offset
    n = fs_offset_l2p(fs, fsfd, l_size);        //** Convert it to the physical offset
    if (l_size > 0)
        n++;                    //** convert the physical offset to a length
...
osd_off_t chksum_merged_write(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t block, osd_off_t offset,
                              osd_off_t len, buffer_t data)
{
    ...
    if ((herr == 0) && (block_bytes_used > 0)) {        //** If it's a valid with data
        ...
        if (n2 != 0) {
            d = block_bytes_used;
            log_printf(0,
                       "chksum_merged_write(%p, " I64T ", " I64T
                       ") original chksum mismatch! memcmp = " I64T " bbu=" I64T " off=" I64T
    ...
    if (n != len) {
        d = block_bytes_used;
        log_printf(0,
                   "chksum_merged_write(%p, " I64T ", " I64T ") error writing data! off=" I64T
                   " len=" I64T " n=" I64T " errno=%d\n", fsfd, obj_offset, ocs->blocksize, offset,
                   len, n, errno);
        n2 = (block == 0) ? OSD_STATE_BAD_HEADER : OSD_STATE_BAD_BLOCK;
        return (n2);
    }
...
osd_off_t chksum_full_write(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t block, buffer_t buffer)
{
    ...
    if ((n != ocs->blocksize) || (herr != 0)) {
        log_printf(0,
                   "chksum_full_write(%p, " I64T ", " I64T ") write error = %d n=" I64T
                   " should be=" I64T " herr=%d\n", fsfd, obj_offset, ocs->blocksize, errno, n,
                   ocs->blocksize, herr);
...
osd_off_t fs_chksum_write(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t offset, osd_off_t len,
                          buffer_t buf)
{
    ...
    if (start_block == 0) {
        ...
        start_off =
            FS_MAGIC_HEADER + ocs->hbs_with_chksum + (start_block - 1) * ocs->bs_with_chksum;
    }
    ...
    log_printf(10,
               "fs_chksum_write(%p, " I64T ", " I64T "): sb=" I64T " eb=" I64T " so=" I64T " ss="
               I64T " es=" I64T " hbs=" I64T " bs=" I64T "\n", fsfd, offset, len, start_block,
               end_block, start_offset, start_size, end_size,
               fsfd->obj->fd_chksum.header_blocksize, fsfd->obj->fd_chksum.blocksize);
    ...
    do {
        ...
        log_printf(10,
                   "fs_chksum_write(%p, " I64T ", " I64T ") sb=" I64T " eb=" I64T " end_got=" I64T
        ...
        for (block = start_block; block <= got_last; block++) {
            ...
            if (n != 0) {
                log_printf(0,
                           "fs_chksum_write(%p, " I64T ", " I64T ") write error = %d n=" I64T "\n",
                           fsfd, offset, len, errno, n);
                err = n;
            }
    ...
    if (end_block != 0)
        posix_fadvise(fileno(fsfd->fd), start_off, doff, POSIX_FADV_DONTNEED);
                          ...
                          buffer_t buffer)
{
    ...
    if (offset > 4095)
        posix_fadvise(fileno(fsfd->fd), offset, len, POSIX_FADV_DONTNEED);
...
osd_off_t chksum_direct_read(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t block, osd_off_t offset,
                             osd_off_t len, buffer_t data_ptr)
{
...
osd_off_t chksum_merged_read(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t block, osd_off_t offset,
                             osd_off_t len, buffer_t data_ptr)
{
    ...
    if (block_bytes_used > 0) { //** Valid chksum stored
        if ((n != bs) && (herr != 0)) {
            log_printf(0,
                       "chksum_merged_read(%p, " I64T ", " I64T ") read error = %d n=" I64T
                       " should be=" I64T " herr=%d\n", fsfd, obj_offset, ocs->blocksize, errno, n,
                       bs, herr);
            n = (block == 0) ? OSD_STATE_BAD_HEADER : OSD_STATE_BAD_BLOCK;
            return (n);
...
osd_off_t fs_chksum_read(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t offset, osd_off_t len,
                         buffer_t buf)
{
    ...
    log_printf(10,
               "fs_chksum_read(%p, " I64T ", " I64T "): sb=" I64T " eb=" I64T " so=" I64T " ss="
               I64T " es=" I64T "\n", fsfd, offset, len, start_block, end_block, start_offset,
               start_size, end_size);
    ...
    do {
        ...
        log_printf(10,
                   "fs_chksum_read(%p, " I64T ", " I64T ") sb=" I64T " eb=" I64T " end_got=" I64T
                   "\n", fsfd, offset, len, start_block, end_block, end_got);
        ...
        if (bpos == 0) {
            if (fs_direct_cache_read(fs->cache, fsfd->obj->id, start_block) == 1) {
                ...
                if (n == 0)
                    fs_cache_add(fs->cache, fsfd->obj->id, start_block);
            }
            if (n != 0) {
                log_printf(0,
                           "fs_chksum_read(%p, " I64T ", " I64T ") read error = %d n=" I64T "\n",
                           fsfd, offset, len, errno, n);
                err = n;
            }
            start_block++;
...
int fs_chksum_info(osd_t *d, osd_id_t id, int *cs_type, osd_off_t *header_blocksize,
                   osd_off_t *blocksize)
{
    osd_fs_chksum_t *fcs;
    int err;
...
osd_off_t fs_get_chksum(osd_t *d, osd_id_t id, char *disk_buffer, char *calc_buffer,
                        ...
                        osd_off_t start_block, osd_off_t end_block)
{
    ...
    if (start_block == 0) {
        ...
        if (n != sizeof(uint32_t)) {
            log_printf(0,
                       "fs_get_chksum: Error reading header block_bytes_used! id=" LU " n=" I64T
                       "\n", id, n);
        }
        ...
        if ((good_block != NULL) || (calc_buffer != NULL)) {
            state =
                _fs_calculate_chksum(fs, fsfd, &(fsfd->chksum), start_block, chksum_buffer,
                                     FS_BUF_SIZE, calc_value, 0);
            if (good_block != NULL)
                good_block[slot] = state;
            if (calc_buffer != NULL)
                memcpy(&(calc_buffer[bpos]), calc_value, cs_size);
        }
    ...
    do {
        ...
        for (block = start_block; block <= end_got; block++) {
            ...
            if (n != sizeof(uint32_t)) {
                log_printf(0,
                           "fs_get_chksum: Error reading block block_bytes_used! id=" LU " block="
                           I64T "\n", id, block);
            }
            ...
            if (n != cs_size) {
                log_printf(0,
                           "fs_get_chksum: Error reading block chksum! id=" LU " block=" I64T
                           " err=" I64T "\n", id, block, n);
            }
            ...
            if ((good_block != NULL) || (calc_buffer != NULL)) {
                state =
                    _fs_calculate_chksum(fs, fsfd, &(fsfd->chksum), block, chksum_buffer,
                                         FS_BUF_SIZE, calc_value, 0);
                if (state != 0)
                    flag_corrupt = 1;
                if (good_block != NULL)
                         ...
                         char *buffer, osd_off_t bufsize, char *calc_value, int correct_errors)
{
    ...
    if (block_bytes_used > 0) { //** Valid chksum stored
        ...
        if (calc_value != NULL)
            memcpy(calc_value, cs_value, cs_size);

        n = memcmp(disk_value, cs_value, cs_size);
        if (n != 0) {
            nblocks = block_bytes_used;
            log_printf(10,
                       "_fs_calculate_chksum mismatch! memcmp = " I64T " id=" LU " offset=" I64T
            ...
            if (correct_errors == 1) {  //** Correct it if they want
                ...
                if (n != cs_size) {
                    log_printf(0,
                               ...
                               I64T "\n", n);
                    if (err == 0)
                        return (3);     //** err could be -1 due to a blocksize error so the chksum could be using the wrong size
                }
...
int fs_validate_chksum(osd_t *d, osd_id_t id, int correct_errors)
{
    ...
    do {
        ...
        for (block = start_block; block <= end_got; block++) {
            err =
                _fs_calculate_chksum(fs, fsfd, &(fsfd->chksum), block, buffer, FS_BUF_SIZE, NULL,
                                     correct_errors);
            if (err != 0) {
...
int fs_trash_iterator_next(osd_iter_t *oi, osd_id_t *id, ibp_time_t *move_time, char *trash_id)
{
    ...
    do {
        ...
        if ((result == NULL) || (n != 0)) {     //** We're finished
            ...
            if (trash_id != NULL)
                strncpy(trash_id, result->d_name, iter->fs->pathlen - 1);
...
void *fs_shelf_object_new(void *arg, int size)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        obj->read_range_list =
            tbx_pc_new("read_range", FS_RANGE_COUNT, sizeof(osd_fs_range_t), NULL,
                       fs_shelf_range_new, fs_shelf_range_free);
        obj->write_range_list =
            tbx_pc_new("write_range", FS_RANGE_COUNT, sizeof(osd_fs_range_t), NULL,
                       fs_shelf_range_new, fs_shelf_range_free);
...
osd_t *osd_mount_fs(const char *device, int n_cache, int n_partitions, apr_time_t expire_time)
{
    ...
    fs->obj_list =
        tbx_pc_new("obj_list", FS_OBJ_COUNT, sizeof(osd_fs_object_t), (void *) fs,
                   fs_shelf_object_new, fs_shelf_object_free);
    assert_result_not_null(fs->obj_list);
    fs->fd_list =
        tbx_pc_new("fd_list", FS_OBJ_COUNT, sizeof(osd_fs_fd_t), (void *) fs, fs_shelf_fd_new,
                   fs_shelf_fd_free);
    assert_result_not_null(fs->fd_list);
    fs->obj_hash = apr_hash_make(fs->pool);
    assert_result_not_null(fs->obj_hash);
