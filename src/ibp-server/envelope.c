...
void envelope_set_command(envelope_t *env, env_command_t cmd)
{
    env->cmd = cmd;
}
...
{
    cmd->u32.byte[0] = val % 256;
    val = val >> 8;
    cmd->u32.byte[1] = val % 256;
    val = val >> 8;
    cmd->u32.byte[2] = val % 256;
    val = val >> 8;
    cmd->u32.byte[3] = val;
}
...
{
    return (cmd->u32.byte[0] + 256 * cmd->u32.byte[1] + 65536 * cmd->u32.byte[2] +
            16777216 * cmd->u32.byte[3]);
}
...
int env_cmd_within_subnet(env_command_t *cmd, env_command_t *cmd_net, int bits)
{
    ...
    n = bits / 8;
    for (i = 0; i < (n - 1); i++) {
        if (cmd->u32.byte[i] != cmd_net->u32.byte[i])
            return (1);
    }
