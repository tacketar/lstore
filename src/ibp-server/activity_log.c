...
int _alog_append_cap(int port, int family, const char *address, const char *key,
                     const char *typekey)
{
    _alog_cap_t a;
    int n;
...
int _alog_read_cap(activity_log_t *alog, int *port, int *family, char *address, char *key,
                   char *typekey)
{
    _alog_cap_t a;
    int n;
...
int _alog_append_alias_copy32(int cmd, int tid, int ri, osd_id_t pid, osd_id_t id, uint64_t size,
                              uint64_t offset, uint64_t off2, int write_mode, int ctype,
                              char *path, int port, int family, const char *address,
                              const char *key, const char *typekey)
{
    ...
    awrite_ul(_alog->fd, &offset2, sizeof(offset2),
              "alog_append_alias_copy32: Error with write!\n");
    _alog_append_cap(port, family, address, key, typekey);
    if (ctype != IBP_TCP)
        _alog_append_string16(strlen(path) + 1, path);
...
int alog_read_alias_copy32(activity_log_t *alog, int cmd, FILE *outfd)
{
    ...
    if (outfd != NULL) {
        ...
        fprintf(outfd,
                "CMD:%s ctype:%s %s wmode:%s RID:%s pid:" LU " id:" LU " offset:" LU " size:" LU
                " host:%s:%d key:%s typekey:%s offset2:" LU "\n", _ibp_cmd_map[ca.cmd],
                _ibp_ctype_map[ca.ctype], path, _ibp_wmode_map[ca.wmode], alog->rl_map[a.ri].name,
...
int _alog_append_alias_copy64(int cmd, int tid, int ri, osd_id_t pid, osd_id_t id, uint64_t size,
                              uint64_t offset, uint64_t offset2, int write_mode, int ctype,
                              char *path, int port, int family, const char *address,
                              const char *key, const char *typekey)
{
    ...
    awrite_ul(_alog->fd, &offset2, sizeof(offset2),
              "alog_append_alias_copy64: Error with write!\n");
    _alog_append_cap(port, family, address, key, typekey);
    if (ctype != IBP_TCP)
        _alog_append_string16(strlen(path) + 1, path);
...
int _alog_append_copy64(int cmd, int tid, int ri, osd_id_t id, uint64_t size,
                        uint64_t offset, uint64_t offset2, int write_mode, int ctype, char *path,
                        int port, int family, const char *address, const char *key,
                        const char *typekey)
{
...
int alog_read_copy64(activity_log_t *alog, int cmd, FILE *outfd)
{
    ...
    if (outfd != NULL) {
        ...
        fprintf(outfd, "CMD:%s ctype:%s %s wmode:%s RID:%s id:" LU " offset:" LU " size:" LU
                " host:%s:%d key:%s typekey:%s offset2:" LU "\n",
                _ibp_cmd_map[ca.cmd], _ibp_ctype_map[ca.ctype], path, _ibp_wmode_map[ca.wmode],
                alog->rl_map[a.ri].name, a.id, a.offset, a.size, host, port, key, typekey, offset2);
    }
...
int alog_read_copy32(activity_log_t *alog, int cmd, FILE *outfd)
{
    ...
    if (outfd != NULL) {
        ...
        fprintf(outfd, "CMD:%s ctype:%s %s wmode:%s RID:%s id:" LU " offset:" LU " size:" LU
                " host:%s:%d key:%s typekey:%s offset2:" LU "\n",
                _ibp_cmd_map[ca.cmd], _ibp_ctype_map[ca.ctype], path, _ibp_wmode_map[ca.wmode],
                alog->rl_map[a.ri].name, a.id, offset, size, host, port, key, typekey, offset2);
    }
...
int alog_append_dd_copy(int cmd, int tid, int ri, osd_id_t pid, osd_id_t id,
                        uint64_t size, uint64_t offset, uint64_t offset2, int write_mode,
                        int ctype, char *path, int port, int family, const char *address,
                        const char *key, const char *typekey)
{
    ...
    if ((size < UINT32_MAX) && (offset < UINT32_MAX) && (offset2 < UINT32_MAX)) {
        if (pid == 0) {
            return (_alog_append_copy32
                    (cmd, tid, ri, id, size, offset, offset2, write_mode, ctype, path, port, family,
                     address, key, typekey));
        } else {
            return (_alog_append_alias_copy32
                    (cmd, tid, ri, pid, id, size, offset, offset2, write_mode, ctype, path, port,
                     family, address, key, typekey));
        }
    } else {
        if (pid == 0) {
            return (_alog_append_copy64
                    (cmd, tid, ri, id, size, offset, offset2, write_mode, ctype, path, port, family,
                     address, key, typekey));
        } else {
            return (_alog_append_alias_copy64
                    (cmd, tid, ri, pid, id, size, offset, offset2, write_mode, ctype, path, port,
                     family, address, key, typekey));
        }
    }
...
int _alog_append_alias_read32(int tid, int ri, osd_id_t pid, osd_id_t id, uint64_t offset,
                              uint64_t size)
{
...
int _alog_append_alias_read64(int tid, int ri, osd_id_t pid, osd_id_t id, uint64_t offset,
                              uint64_t size)
{
...
int _alog_append_write_append32(int tid, int cmd, int ri, osd_id_t id, uint64_t offset,
                                uint64_t size)
{
...
int _alog_append_write_append64(int tid, int cmd, int ri, osd_id_t id, uint64_t offset,
                                uint64_t size)
{
...
int _alog_append_alias_write32(int tid, int cmd, int ri, osd_id_t pid, osd_id_t id,
...
int _alog_append_alias_write64(int tid, int cmd, int ri, osd_id_t pid, osd_id_t id,
                               uint64_t offset, uint64_t size)
{
...
int _alog_append_alias_write_append32(int tid, int cmd, int ri, osd_id_t pid, osd_id_t id,
                                      uint64_t offset, uint64_t size)
{
...
int _alog_append_alias_write_append64(int tid, int cmd, int ri, osd_id_t pid, osd_id_t id,
                                      uint64_t offset, uint64_t size)
{
...
int alog_append_write(int tid, int cmd, int ri, osd_id_t pid, osd_id_t id, uint64_t offset,
                      uint64_t size)
{
    alog_mode_check();
...
int alog_read_manage_change(activity_log_t *alog, int cmd, FILE *outfd)
{
    ...
    if (outfd != NULL) {
        ...
        buf[strlen(buf) - 1] = '\0';    //** chomp the \n from ctime_r
        fprintf(outfd,
                "CMD:IBP_MANAGE SUBCMD:IBP_CHNG RID:%s id:" LU " reliablity:%s size:" LU
                " expiration: %s(" TT ")\n", alog->rl_map[a.ri].name, a.id, _ibp_rel_map[a.rel],
                a.size, buf, t);
    }
...
int alog_append_alias_manage_change(int tid, int ri, osd_id_t id, uint64_t offset, uint64_t size,
                                    apr_time_t t)
{
...
int alog_read_alias_manage_change(activity_log_t *alog, int cmd, FILE *outfd)
{
    ...
    if (outfd != NULL) {
        ...
        buf[strlen(buf) - 1] = '\0';    //** chomp the \n from ctime_r
        fprintf(outfd,
                "CMD:IBP_ALIAS_MANAGE SUBCMD:IBP_CHNG RID:%s  id:" LU " offset:" LU " size:" LU
                " expiration: %s(" TT ")\n", alog->rl_map[a.ri].name, a.id, a.offset, a.size, buf,
                t);
    }
...
int _alog_append_pm_incdec(int tid, int cmd, int subcmd, int ri, osd_id_t pid, osd_id_t id,
                           int cap_type)
{
...
int alog_read_alias_manage_incdec(activity_log_t *alog, int cmd, FILE *outfd)
{
    ...
    if (outfd != NULL) {
        fprintf(outfd, "CMD:IBP_ALIAS_MANAGE SUBCMD:%s RID:%s pid:" LU " id:" LU " cap_type:%s\n",
                _ibp_subcmd_map[a.subcmd], alog->rl_map[a.ri].name, a.pid, a.id,
                _ibp_captype_map[a.captype]);
    }
...
int alog_read_manage_incdec(activity_log_t *alog, int cmd, FILE *outfd)
{
    ...
    if (outfd != NULL) {
        fprintf(outfd, "CMD:IBP_MANAGE SUBCMD:%s RID:%s id:" LU " cap_type:%s\n",
                _ibp_subcmd_map[a.subcmd], alog->rl_map[a.ri].name, a.id,
                _ibp_captype_map[a.captype]);
    }
...
int alog_append_manage_incdec(int tid, int cmd, int subcmd, int ri, osd_id_t pid, osd_id_t id,
                              int cap_type)
{
    alog_mode_check();
...
int alog_read_subcmd(const char *cmd_name, activity_log_t *alog, const char **sub_name,
                     FILE *outfd)
{
    uint8_t a;
...
int _alog_append_alias_alloc32(int tid, int ri, osd_id_t id, uint64_t offset, uint64_t size,
                               apr_time_t expire)
{
...
int alog_read_alias_alloc32(activity_log_t *alog, int cmd, FILE *outfd)
{
    ...
    if (outfd != NULL) {
        ...
        fprintf(outfd,
                "CMD:IBP_ALIAS_ALLOCATE  RID:%s id:" LU " offset:" LU " size:" LU " expire:%s(" TT
...
int _alog_append_alias_alloc64(int tid, int ri, osd_id_t id, uint64_t offset, uint64_t size,
                               apr_time_t expire)
{
...
int alog_append_alias_alloc(int tid, int ri, osd_id_t id, uint64_t offset, uint64_t size,
                            apr_time_t expire)
{
    alog_mode_check();
...
int alog_append_ibp_allocate(int tid, int rindex, uint64_t max_size, int atype, int rel,
...
int alog_read_ibp_allocate(activity_log_t *alog, int cmd, FILE *outfd)
{
    ...
    if (outfd != NULL) {
        ...
        fprintf(outfd,
                "CMD:IBP_ALLOCATE RID:%s type:%s rel:%s size=" LU " expiration:%s(" TT ")\n",
...
int alog_append_ibp_split_allocate(int tid, int rindex, osd_id_t mid, uint64_t max_size, int atype,
                                   int rel, apr_time_t expiration)
{
    void *d;
    int nbytes, cmd;
    _alog_alloc64_t a64;
...
int alog_read_ibp_split_allocate(activity_log_t *alog, int cmd, FILE *outfd)
{
    ...
    if (outfd != NULL) {
        ...
        buffer[strlen(buffer) - 1] = '\0';      //** chomp the \n from ctime_r
        fprintf(outfd,
                "CMD:IBP_SPLIT_ALLOCATE MID:" LU " RID:%s type:%s rel:%s size:" LU
                " expiration:%s(" TT ")\n", mid, alog->rl_map[ri].name, _ibp_type_map[atype],
                _ibp_rel_map[rel], msize, buffer, t);
    }
...
int alog_append_cmd_result(int tid, int status)
{
    ...
    n8 = -status;
    if (status > 0)
        n8 = 0;                 // ** IBP_OK = 1 but there is no 0 error
...
int _alog_append_thread_open(int tid, int ns_id, int family, char *address)
{
    ...
    n16 = tid;
    awrite_ul(_alog->fd, &n16, sizeof(n16), "alog_append_thread_open: Error with write!\n");
    n32 = ns_id;
    awrite_ul(_alog->fd, &n32, sizeof(n32), "alog_append_thread_open: Error with write!\n");
...
int _alog_transfer_data(char *fname)
{
    ...
    if (err != NS_OK) {
        fclose(alog.fd);
        log_printf(0,
                   "_alog_transfer_Data: server_ns_write_block failed for sending %s header! err=%d n=%d\n",
                   fname, err, n);
        tbx_ns_destroy(ns);
        return (5);
    }
    ...
    if (err != NS_OK) {
        fclose(alog.fd);
        log_printf(0,
                   "_alog_transfer_Data: envelope_simple_recv failed reading response header! err=%d n=%d\n",
                   err, n);
        tbx_ns_destroy(ns);
        return (6);
    }
    ...
    if (err != NS_OK) {
        log_printf(0,
                   "_alog_transfer_Data: envelope_simple_recv failed reading response after send! err=%d n=%d\n",
                   err, n);
        tbx_ns_destroy(ns);
        return (6);
    }
    if (response != 1) {
        err = response;
        log_printf(0,
                   "_alog_transfer_Data: alog_server returned an error with data! response=%d\n",
                   err);
        tbx_ns_destroy(ns);
        return (6);
    }
...
void _alog_nsmap(ns_map_t *nsmap, int n)
{
    ...
    for (i = 0; i < n; i++) {
        if (nsmap[i].used == 1)
            _alog_append_thread_open(i, nsmap[i].id, nsmap[i].family, nsmap[i].address);
    }
}
...
//  alog_lock();
    ...
    if (_alog_pending_stack == NULL)
        _alog_pending_stack = tbx_stack_new();
...
//log_printf(0, "next_entry: header block: (%d,%d) * cmd=%d %d\n", d1, d2, command, errno);
    ...
    if ((alog->ns_map != NULL) && (id < alog->max_id))
        id = alog->ns_map[id].id;

    if ((command == ALOG_REC_OPEN) || (command == ALOG_REC_CLOSE))
        id = 0;                 //** Open commands *determine* the header size so this could be bogus
    ...
    if (alog->table[command].process_entry != NULL) {
        ...
        if (fd != NULL)
            fprintf(fd, "Invalid header! t=" TT " * id=%d command=%d!!!!!!!!!!!!!!!!\n", t, id,
                    command);
        return (-2);
    }
...
int write_file_header(activity_log_t *alog)
{
    ...
    n = h->version;
    awrite(alog->fd, &n, sizeof(n), "write_file_header: Error storing version\n");      //** Version
    n = h->start_time;
    awrite(alog->fd, &n, sizeof(n), "write_file_header: Error storing start_time\n");   //** Start time
    n = h->end_time;
    awrite(alog->fd, &n, sizeof(n), "write_file_header: Error storing end time\n");     //** End time
    n = h->state;
    awrite(alog->fd, &n, sizeof(n), "write_file_header: Error storing state\n");        //** Mark the state as in use
...
int update_file_header(activity_log_t *alog, int state)
{
    ...
    if (state == STATE_BAD) {
        ...
        awrite(alog->fd, &n, sizeof(n), "update_file_header: Error setting end time\n");        //** End time
        n = STATE_GOOD;
        awrite(alog->fd, &n, sizeof(n), "update_file_header: Error setting STATE_GOOD\n");      //** Mark the state as in use
    }
...
activity_log_t *activity_log_open(const char *logname, int max_id, int mode)
{
    ...
    if (mode == ALOG_READ) {
        ...
        if (alog->fd == NULL) {
            ...
            if (alog->header.state == STATE_BAD) {
                log_printf(0,
                           "activity_log_open: alog %s was not closed properly! Moving to EOF manually.\n",
                           alog->name);
