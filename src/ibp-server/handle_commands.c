...
int handle_allocate(ibp_task_t *task)
{
    ...
    if ((cmd->command == IBP_SPLIT_ALLOCATE) || (cmd->command == IBP_SPLIT_ALLOCATE_CHKSUM)) {
        ...
        alog_append_ibp_allocate(task->myid, res->rl_index, alloc->max_size, alloc->type,
                                 alloc->reliability, alloc->expiration);
    }
    ...
    if ((cmd->command == IBP_SPLIT_ALLOCATE) || (cmd->command == IBP_SPLIT_ALLOCATE_CHKSUM)) {
        if ((d =
                 split_allocation_resource(res, &ma, &a, alloc->max_size, alloc->type,
                                           alloc->reliability, alloc->expiration, 0, d, ca->cs_type,
                                           ca->cs_blocksize)) != 0) {
            log_printf(1,
                       "handle_allocate: split_allocation_resource failed on RID %s!  Error=%d\n",
                       res->name, d);
        ...
        update_manage_history(res, ma.id, ma.is_alias, &(task->ipadd), cmd->command, 0,
                              a.reliability, a.expiration, a.max_size, 0);

        unlock_osd_id(ma.id);   //** Now we can unlock the master
    } else {
        if ((d =
                 create_allocation_resource(res, &a, alloc->max_size, alloc->type, alloc->reliability,
                                            alloc->expiration, 0, d, ca->cs_type,
                                            ca->cs_blocksize)) != 0) {
            log_printf(1,
...
int handle_merge(ibp_task_t *task)
{
    ...
    update_manage_history(r, ca.id, ca.is_alias, &(task->ipadd), cmd->command, 0, 0, 0,
                          ca.max_size, 0);
...
int handle_internal_get_alloc(ibp_task_t *task)
{
    ...
    if ((cs_type != CHKSUM_NONE) && (err == 0)) {
        ...
        if ((a.size % blocksize) > 0)
            nblocks++;          //** Handle a partial block
    ...
    //*** Send back the results ***
    sprintf(buffer, "%d " I64T " %d %d " I64T " " I64T " " LU " \n", IBP_OK, nblocks, state,
            cs_type, hbs, blocksize, nbytes);
    err = server_ns_write_block(task->ns, task->cmd_timeout, buffer, strlen(buffer));
...
int handle_alias_allocate(ibp_task_t *task)
{
    ...
    if (pa->expiration > a.expiration) {
        log_printf(1,
                   "handle_alias_alloc: ALIAS duration > actual allocation! alias= %u alloc = %u\n",
                   pa->expiration, a.expiration);
    ...
    if ((pa->len + pa->offset) > (int64_t) a.max_size) {
        ...
        log_printf(1,
                   "handle_alias_alloc: ALIAS range > actual allocation! alias= " LU " alloc = " LU
                   "\n", epos, a.max_size);
        send_cmd_result(task, IBP_E_WOULD_EXCEED_LIMIT);
        unlock_osd_id(a.id);
    ...
    //*** Create the alias ***
    if ((d =
             create_allocation_resource(res, &alias_alloc, 0, a.type, a.reliability, pa->expiration, 1,
                                        0, 0, 0)) != 0) {
        log_printf(1,
                   "handle_alias_alloc: create_allocation_resource failed on RID %s!  Error=%d\n",
                   res->name, d);
        send_cmd_result(task, IBP_E_GENERIC);
        unlock_osd_id(a.id);
    ...
    update_manage_history(res, a.id, a.is_alias, &(task->ipadd), IBP_ALIAS_ALLOCATE, 0,
                          alias_alloc.alias_offset, alias_alloc.expiration, alias_alloc.alias_size,
                          alias_alloc.id);
...
int handle_status(ibp_task_t *task)
{
    ...
    if (status->subcmd == IBP_ST_VERSION) {
        ...
        tbx_stk_strncat(result, version, sizeof(result) - 1 - strlen(result), sizeof(result));

        //** Add info on the interfaces
        sprintf(buffer, "Interfaces(%d): ", global_config->server.n_iface);
        tbx_stk_strncat(result, buffer, sizeof(result) - 1 - strlen(result), sizeof(result));
        ...
        tbx_stk_strncat(result, "\n", sizeof(result) - 1 - strlen(result), sizeof(result));
        ...
        while ((r = resource_list_iterator_next(global_config->rl, &it)) != NULL) {
            ...
            r_alloc = r->n_allocs;
            r_alias = r->n_alias;
            del_free = r->trash_size[RES_DELETE_INDEX];
            exp_free = r->trash_size[RES_EXPIRE_INDEX];
            n_del = r->n_trash[RES_DELETE_INDEX];
            n_exp = r->n_trash[RES_EXPIRE_INDEX];
            ...
            del_total = del_total + del_free;
            exp_total = exp_total + exp_free;
...
//        r_free_gb = total_free / (1024.0 * 1024.0 * 1024.0);
    ...
    if (status->subcmd == IBP_ST_RES) {
        ...
        if (totalused > totalconfigured) {
            ...
            if (soft_alloc > total_alloc)
                soft_alloc = total_alloc;
            hard_alloc = r->max_size[ALLOC_HARD] - r->used_space[ALLOC_HARD];
            if (hard_alloc > total_alloc)
                hard_alloc = total_alloc;
        }
        ...
        if (cmd->version == IBPv040) {
            ...
            soft = r->max_size[ALLOC_SOFT] >> 20;
            soft_used = r->used_space[ALLOC_SOFT] >> 20;
            hard = r->max_size[ALLOC_HARD] >> 20;
            hard_used = r->used_space[ALLOC_HARD] >> 20;
...
//==================IBP_ST_CHANGE NOT ALLOWED==============================
        ...
        log_printf(10,
                   "handle_status: Requested change on RID %s hard:" LU " soft:" LU
                   " expiration:%ld\n", r->name, status->new_size[ALLOC_HARD],
                   status->new_size[ALLOC_SOFT], status->new_duration);

        apr_thread_mutex_lock(r->mutex);
        if (r->used_space[ALLOC_HARD] < status->new_size[ALLOC_HARD])
            r->max_size[ALLOC_HARD] = status->new_size[ALLOC_HARD];
        if (r->used_space[ALLOC_SOFT] < status->new_size[ALLOC_SOFT])
            r->max_size[ALLOC_SOFT] = status->new_size[ALLOC_SOFT];
        if (status->new_duration < 0)
            status->new_duration = INT_MAX;
        r->max_duration = status->new_duration;
        apr_thread_mutex_unlock(r->mutex);

        log_printf(10,
                   "handle_status: Succesfully processed IBP_ST_CHANGE on RID %s hard:" LU " soft:"
                   LU " expiration:%d\n", r->name, r->max_size[ALLOC_HARD],
                   r->max_size[ALLOC_SOFT], r->max_duration);
...
int handle_manage(ibp_task_t *task)
{
    ...
    if ((a->is_alias == 1) && (cmd->command == IBP_MANAGE)) {   //** This is a alias cap so load the master and invoke restrictions
        ...
        if (alias_len == 0)
            alias_len = a->max_size - alias_offset;
    ...
    //@fallthrough@
    case IBP_DECR:
        alog_append_manage_incdec(task->myid, cmd->command, manage->subcmd, r->rl_index, pid, id,
                                  manage->captype);
        err = IBP_OK;
    ...
    case IBP_TRUNCATE:
        ...
        if (is_alias == 1) {    //** If this is an IBP_ALIAS_MANAGE call we have different fields
            ...
            alog_append_alias_manage_change(task->myid, r->rl_index, a->id, a->alias_offset,
                                            a->alias_size, a->expiration);
        } else {
        ...
        if (is_alias == 1) {
            update_manage_history(r, a->id, a->is_alias, &(task->ipadd), cmd->command,
                                  manage->subcmd, a->alias_offset, a->expiration, a->alias_size,
                                  pid);
        } else {
            update_manage_history(r, a->id, a->is_alias, &(task->ipadd), cmd->command,
    ...
    case IBP_CHNG:
        ...
        if (is_alias == 1) {    //** If this is an IBP_ALIAS_MANAGE call we have different fields
            ...
            if (manage->new_duration >= 0) {
                if (manage->new_duration == INT_MAX)
                    manage->new_duration = ibp_time_now() + r->max_duration;
        ...
        break;
    case IBP_PROBE:
        if (a->reliability == ALLOC_HARD)
            rel = IBP_HARD;

        if (cmd->command == IBP_MANAGE) {
            alog_append_manage_probe(task->myid, r->rl_index, id);
            log_printf(15,
                       "handle_manage: poffset=" LU " plen=" LU " a->size=" LU " a->max_size=" LU
                       " ns=%d\n", alias_offset, alias_len, a->size, a->max_size,
                       tbx_ns_getid(task->ns));
            pmax_size = a->max_size - alias_offset;
            if (pmax_size > alias_len)
                pmax_size = alias_len;
            ...
            snprintf(buf, sizeof(buf) - 1, "%d %d %d " LU " " LU " %ld %d %d \n",
                     IBP_OK, a->read_refcount, a->write_refcount, psize, pmax_size,
                     a->expiration - ibp_time_now(), rel, a->type);
        } else {                //** IBP_ALIAS_MANAGE
            alog_append_alias_manage_probe(task->myid, r->rl_index, pid, id);
            snprintf(buf, sizeof(buf) - 1, "%d %d %d " LU " " LU " %ld \n",
                     IBP_OK, a->read_refcount, a->write_refcount, a->alias_offset, a->alias_size,
                     a->expiration - ibp_time_now());
        }
...
int handle_validate_chksum(ibp_task_t *task)
{
    ...
    if (w->r == NULL) {
        log_printf(10, "handle_validate_chksum:  Invalid RID :%s  tid=" LU "\n", w->crid,
                   task->tid);
        alog_append_write(task->myid, cmd->command, w->r->rl_index, pid, id, w->iovec.vec[0].off,
                          w->iovec.vec[0].len);
...
int handle_get_chksum(ibp_task_t *task)
{
    ...
    log_printf(10,
               "handle_get_chksum: Starting to process command tid=" LU " ns=%d chksum_only=" I64T
...
//not counted in a->size  nblocks--; //** Don't send the header block
    ...
    snprintf(buffer, sizeof(buffer), "%d %d %d " I64T " " I64T " " I64T "\n", IBP_OK, cs_type,
             cs_size, blocksize, nblocks, nbuf);
...
//  nblocks++;  //** Add it back in for the loop
    ...
    for (i = 1; i < nblocks; i = i + nbuf) {
        ...
        err =
            get_allocation_chksum(w->r, a->id, buffer, NULL, bufsize, NULL, NULL, start_block,
                                  end_block);
        if (err != csbytes) {
...
int handle_write(ibp_task_t *task)
{
    ...
    debug_printf(1,
                 "handle_write: Starting to process command tid=" LU " ns=%d chksum_enable=%d\n",
    ...
    if (task->enable_chksum == 1) {
        tbx_ns_chksum_read_set(task->ns, task->ncs);
        tbx_ns_chksum_read_enable(task->ns);
    }
    ...
    if (a->is_alias == 1) {     //** This is a alias allocation so load the actual one
        ...
        if ((err = get_allocation_resource(w->r, id, a)) != 0) {
            alog_append_write(task->myid, cmd->command, w->r->rl_index, pid, id,
                              w->iovec.vec[0].off, w->iovec.vec[0].len);
            log_printf(10,
    ...
    if (err == -1) {            //** Dead connection
        ...
        update_write_history(w->r, a->id, a->is_alias, &(task->ipadd), w->iovec.vec[0].off,
                             w->iovec.vec[0].len, pid);
...
int handle_read(ibp_task_t *task)
{
    ...
    log_printf(10, "handle_read: id: " LU " is_alias=%d cmd offset=" OT " ns=%d\n", a->id,
               a->is_alias, r->iovec.vec[0].off, tbx_ns_getid(task->ns));
    ...
    if (a->is_alias == 1) {     //** This is a alias allocation so load the actual one
        ...
        log_printf(10, "handle_read: mid: " LU " offset=" I64T " len=" I64T " ns=%d\n", id,
                   alias_offset, alias_len, tbx_ns_getid(task->ns));

        if ((err = get_allocation_resource(r->r, id, a)) != 0) {
            log_printf(10,
                       "handle_read: Invalid alias_id: " LU " for resource = %s  tid=" LU
        ...
        alog_append_read(task->myid, r->r->rl_index, 0, pid, r->iovec.vec[0].off,
                         r->iovec.vec[0].len);
    }
    ...
    if (a->type == IBP_BYTEARRAY) {
        if (task->enable_chksum == 1) {
            tbx_ns_chksum_write_set(task->ns, task->ncs);
            tbx_ns_chksum_write_enable(task->ns);
        }
        err = 0;
    ...
    lock_osd_id(a->id);
    update_read_history(r->r, a->id, a->is_alias, &(task->ipadd), r->iovec.vec[0].off,
                        r->iovec.vec[0].len, pid);
    unlock_osd_id(a->id);
...
int same_depot_copy(ibp_task_t *task, char *rem_cap, char *rem_id, int rem_offset, osd_id_t rpid)
{
    ...
    if (rem_offset < 0)
        rem_offset = rem_a.size;        //** This is an append copy
    ...
    if (((rem_offset + r->iovec.vec[0].len) > (int64_t) rem_a.max_size)
...
//---------------
    ...
    if (r->transfer_dir == IBP_PULL) {
        ...
        dest_a = a;
        dest_r = r->r;
        doff = r->iovec.vec[0].off;
        dpid = rpid;
    } else {
        src_a = a;
        src_r = r->r;
        soff = r->iovec.vec[0].off;
        spid = rpid;
        dest_a = &rem_a;
        dest_r = rem_r;
        doff = rem_offset;
    ...
    log_printf(15, "same_depot_copy: rem_offset=%d r->offset=" OT " ns=%d\n", rem_offset,
               r->iovec.vec[0].off, tbx_ns_getid(task->ns));

    err =
        disk_to_disk_copy(src_r, src_a->id, soff, dest_r, dest_a->id, doff, r->iovec.vec[0].len,
                          task->cmd_timeout);
    if (err != IBP_OK) {
        send_cmd_result(task, err);
    ...
    soff = soff + r->iovec.vec[0].len;
    doff = doff + r->iovec.vec[0].len;
    ...
    update_write_history(dest_r, dest_a->id, dest_a->is_alias, &(task->ipadd), dest_off,
                         r->iovec.vec[0].len, dpid);
    ...
    update_read_history(src_r, src_a->id, src_a->is_alias, &(task->ipadd), src_off,
                        r->iovec.vec[0].len, spid);
    ...
    log_printf(10, "same_depot_copy: ns=%d Completed successfully.  Sending result: %s",
               tbx_ns_getid(task->ns), result);
    server_ns_write_block(task->ns, task->cmd_timeout, result, strlen(result));
    log_printf(15,
               "handle_copysend: END pns=%d cns=%d---same_depot_copy-------------------------\n",
...
int handle_copy(ibp_task_t *task)
{
    ...
    log_printf(15, "iovec->n=%d off=" OT " len=" OT " cumulative_len=" OT "\n", iov->n,
               iov->vec[0].off, iov->vec[0].len, iov->vec[0].cumulative_len);
    ...
    strncpy(key, tbx_stk_string_token(NULL, "/", &bstate, &fin), sizeof(key) - 1);
    key[sizeof(key) - 1] = '\0';
    strncpy(typekey, tbx_stk_string_token(NULL, "/", &bstate, &fin), sizeof(typekey) - 1);
    typekey[sizeof(typekey) - 1] = '\0';

    tbx_dnsc_lookup(rhost, addr, NULL);
    alog_append_dd_copy(cmd->command, task->myid, r->r->rl_index, apid, aid, r->iovec.vec[0].len,
                        r->iovec.vec[0].off, r->remote_offset, r->write_mode, r->ctype, r->path,
                        rport, AF_INET, addr, key, typekey);

    log_printf(15, "handle_copy: rhost=%s rport=%d cap=%s key=%s typekey=%s\n", rhost, rport,
               r->remote_cap, key, typekey);
    ...
    if (apr_time_sec(to) > 20)
        to = apr_time_make(20, 0);      //** If no connection in 20 sec die anyway
    if (apr_time_sec(to) <= 0)
        to = apr_time_make(1, 0);
...
int handle_transfer(ibp_task_t *task, osd_id_t rpid, tbx_ns_t *ns, const char *key,
                    const char *typekey)
{
    Cmd_state_t *cmd = &(task->cmd);
    Cmd_read_t *r = &(cmd->cargs.read);
    ...
    case IBP_PUSH:
        if (r->write_mode == 1) {
            snprintf(write_cmd, 1024, "%d %d %s %s " OT " %d \n", IBPv040, IBP_STORE, key, typekey,
                     rlen, rto);
        } else {
            snprintf(write_cmd, 1024, "%d %d %s %s " OT " " OT " %d \n", IBPv040, IBP_WRITE, key,
                     typekey, r->remote_offset, rlen, rto);
        }
        break;
    case IBP_PUSH_CHKSUM:
        if (r->write_mode == 1) {
            snprintf(write_cmd, 1024, "%d %d %d " I64T " %s %s " OT " %d \n", IBPv040,
                     IBP_STORE_CHKSUM, tbx_ns_chksum_type(&(task->ncs)),
                     tbx_ns_chksum_blocksize(&(task->ncs)), key, typekey, rlen, rto);
        } else {
            snprintf(write_cmd, 1024, "%d %d %d " I64T " %s %s " OT " " OT " %d \n", IBPv040,
                     IBP_WRITE_CHKSUM, tbx_ns_chksum_type(&(task->ncs)),
                     tbx_ns_chksum_blocksize(&(task->ncs)), key, typekey, r->remote_offset, rlen,
                     rto);
        }
        break;
    case IBP_PULL:
        snprintf(write_cmd, 1024, "%d %d %s %s " OT " " OT " %d \n", IBPv040, IBP_LOAD, key,
                 typekey, r->remote_offset, rlen, rto);
        break;
    case IBP_PULL_CHKSUM:
        snprintf(write_cmd, 1024, "%d %d %d " I64T " %s %s " OT " " OT " %d \n", IBPv040,
                 IBP_LOAD_CHKSUM, tbx_ns_chksum_type(&(task->ncs)),
                 tbx_ns_chksum_blocksize(&(task->ncs)), key, typekey, r->remote_offset, rlen, rto);
        break;
    }
    ...
    if (err != NS_OK) {
        if (task->cmd_timeout > apr_time_now()) {
            log_printf(10,
                       "handle_transfer: TIMEOUT Error sending command! server_ns_write_block=%d  cmd=%s\n",
                       err, write_cmd);
            send_cmd_result(task, IBP_E_SERVER_TIMEOUT);
        } else {
            log_printf(10,
                       "handle_transfer: Error sending command! server_ns_write_block=%d  cmd=%s\n",
                       err, write_cmd);
            send_cmd_result(task, IBP_E_CONNECTION);
        }
        return (0);
...
//asm volatile ("int3;");
    ...
    while (err == 0) {
        ...
        if (err == 0) {
            if (apr_time_now() > (task->cmd_timeout - apr_time_make(1, 0)))
                err = -1;
        }
    }
    ...
    if (err == -1) {            //** Dead connection
        ...
        if (r->transfer_dir == IBP_PUSH) {
            update_read_history(r->r, a->id, a->is_alias, &(task->ipadd), r->iovec.vec[0].off,
                                r->iovec.vec[0].len, rpid);
        } else {                //** And also the size if needed
            update_write_history(r->r, a->id, a->is_alias, &(task->ipadd), r->iovec.vec[0].off,
                                 r->iovec.vec[0].len, rpid);
        ...
        result[511] = '\0';
        log_printf(10, "handle_transfer: ns=%d Completed.  Sending result: %s",
                   tbx_ns_getid(task->ns), result);
        server_ns_write_block(task->ns, task->cmd_timeout, result, strlen(result));
...
int handle_internal_date_free(ibp_task_t *task)
{
    ...
    while ((err == 0) && (arg->size > total_bytes)) {
        if (a.expiration != curr_time) {        //** time change so print the current stats
            if (curr_time != 0) {
                sprintf(text, TT " %d %d " LU " " LU "\n", curr_time, a_count, p_count, bytes_used,
                        bytes);
                log_printf(10, "handle_internal_date_free:  ns=%d sending %s",
                           tbx_ns_getid(task->ns), text);
                err = server_ns_write_block(task->ns, task->cmd_timeout, text, strlen(text));
                if (err != (int) strlen(text)) {
                    log_printf(10,
                               "handle_internal_date_free:  ns=%d erro with server_ns_write=%d\n",
                               tbx_ns_getid(task->ns), err);
                } else {
                    err = 0;
                }
        ...
        log_printf(15,
                   "handle_internal_date_free: ns=%d time=" TT " a_count=%d p_count=%d bytes=" LU
                   " total=" LU "\n", tbx_ns_getid(task->ns), ibp2apr_time(curr_time), a_count,
                   p_count, bytes, total_bytes);
    ...
    if (curr_time != 0) {
        ...
        log_printf(10, "handle_internal_date_free:  ns=%d sending %s", tbx_ns_getid(task->ns),
                   text);
        err = server_ns_write_block(task->ns, task->cmd_timeout, text, strlen(text));
        if (err != (int) strlen(text)) {
...
int handle_internal_expire_list(ibp_task_t *task)
{
    ...
    while ((err == 0) && (i < arg->max_rec)) {
        ...
        log_printf(10, "handle_internal_expire_list:  ns=%d sending %s", tbx_ns_getid(task->ns),
                   text);
        err = server_ns_write_block(task->ns, task->cmd_timeout, text, strlen(text));
        if (err != (int) strlen(text)) {
...
void rid_log_append(char *crid, char *mode, char *result, char *msg, apr_time_t start,
                    apr_time_t end)
{
    FILE *fd;
