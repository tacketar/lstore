    ...
    for (slot = 0; slot < count; slot++) {
        ...
        if (t != 0) {
            ...
            case IBP_MANAGE:
                cmd = "IBP_MANAGE        ";
                break;
            case IBP_ALIAS_MANAGE:
                cmd = "IBP_ALIAS_MANAGE  ";
                break;
            case IBP_RENAME:
                cmd = "IBP_RENAME        ";
                subcmd = "            ";
            ...
            if ((ts->cmd == IBP_ALIAS_ALLOCATE) || (ts->cmd == IBP_ALIAS_MANAGE)) {
                tbx_append_printf(buffer, used, nbytes,
                                  "   " TT " * %s  * %s * " LU " * %s * %s * " LU " * " LU " * " TT
                                  " * %s\n", t, print_time, hostip, ts->id, cmd, subcmd,
                                  ts->reliability, ts->size, t2, print_time2);
            } else {
                tbx_append_printf(buffer, used, nbytes,
                                  "   " TT " * %s  * %s * " LU " * %s * %s * %s * " LU " * " TT
                                  " * %s\n", t, print_time, hostip, ts->id, cmd, subcmd, rel,
                                  ts->size, t2, print_time2);
            }
        }
    }
}
...
void print_rw_history(char *buffer, int *used, int nbytes, Allocation_rw_ts_t *ts_list, int count)
{
    ...
    for (slot = 0; slot < count; slot++) {
        ...
        if (t != 0) {
            ...
            tbx_append_printf(buffer, used, nbytes,
                              "   " TT " * %s * %s * " LU " * " LU " * " LU "\n", t, print_time,
                              hostip, ts->id, ts->offset, ts->size);
        }
    }
}
                      ...
                      Allocation_history_db_t *h, int state, int cs_type, osd_off_t hbs, osd_off_t bs)
{
    ...
    tbx_append_printf(buffer, used, nbytes,
                      "Certifed by %s with CA id: " LU ":" LU ":" LU ":" LU " (", hostip,
                      a->creation_cert.u.id[0], a->creation_cert.u.id[1], a->creation_cert.u.id[2],
                      a->creation_cert.u.id[3]);
    for (i = 0; i < 32 - 1; i++) {
        n = (unsigned char) (a->creation_cert.u.cid[i]);
        tbx_append_printf(buffer, used, nbytes, "%d.", n);
    };
    n = (unsigned char) (a->creation_cert.u.cid[15]);
    tbx_append_printf(buffer, used, nbytes, "%d)\n", n);
    ...
    switch (a->type) {
    case IBP_BYTEARRAY:
        tbx_append_printf(buffer, used, nbytes, "Type: IBP_BYTEARRAY\n");
        break;
    case IBP_BUFFER:
        tbx_append_printf(buffer, used, nbytes, "Type: IBP_BUFFER\n");
        break;
    case IBP_FIFO:
        tbx_append_printf(buffer, used, nbytes, "Type: IBP_FIFO\n");
        break;
    case IBP_CIRQ:
        tbx_append_printf(buffer, used, nbytes, "Type: IBP_CIRQ\n");
        break;
    default:
        tbx_append_printf(buffer, used, nbytes, "Type: (%d) UNKNOWN TYPE!!!! \n", a->type);
        break;
    }

    switch (a->reliability) {
    case ALLOC_HARD:
        tbx_append_printf(buffer, used, nbytes, "Reliability: IBP_HARD\n");
        break;
    case ALLOC_SOFT:
        tbx_append_printf(buffer, used, nbytes, "Reliability: IBP_SOFT\n");
        break;
    default:
        tbx_append_printf(buffer, used, nbytes, "Reliability: (%d) UNKNOWN RELIABILITY!!!\n",
                          a->reliability);
    }
    ...
    if (tbx_chksum_type_valid(cs_type) == 1) {
        ...
        tbx_append_printf(buffer, used, nbytes,
                          "Chksum Type: %s(%d)  Binary length: %d   Hex length: %d\n",
                          tbx_chksum_name(&cs), cs_type, i, n);
        tbx_append_printf(buffer, used, nbytes,
                          "Chksum Header size: " I64T "   Block size: " I64T "\n", hbs, bs);
    } else {
        tbx_append_printf(buffer, used, nbytes, "State: %d\n", state);
