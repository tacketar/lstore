...
void store_depot(ibp_depot_t *depot, char **argv, int skip_rid)
{
    ...
    ibp_depot_set(depot, argv[0], port, rid);
}
...
void store_attr(ibp_attributes_t *attr, char **argv)
{
    ...
    ibp_attributes_set(attr, time(NULL) + duration, rel, type);
}
...
int cmd_validate_chksum(char **argv, int argc)
{
    ...
    if (argc < 3) {
        printf("cmd_validate_chksum: Not enough parameters.  Received %d need 3\n", argc);
        return(0);
    }
    ...
    status = gop_sync_exec_status(ibp_validate_chksum_gop(ic, cap, correct_errors, &n_bad, timeout));
    if (status.error_code != IBP_OK) {
        printf("cmd_validate_chksum: Error %s(%d)\n", _ibp_error_map[-status.error_code], status.error_code);
        return(0);
    }
...
int cmd_get_chksum(char **argv, int argc)
{
    ...
    status = gop_sync_exec_status(ibp_alloc_chksum_get_gop(ic, cap, info_only, &cs_type, &cs_size, &blocksize, &nblocks, &n_chksumbytes, NULL, 0, timeout));
    if (status.error_code != IBP_OK) {
...
int cmd_allocate(char **argv, int argc)
{
    ...
    if (argc < 8) {
        printf("cmd_allocate: Not enough parameters.  Received %d need 8\n", argc);
        return(0);
    }
    ...
    status = gop_sync_exec_status(ibp_alloc_gop(ic, &caps, size, &depot, &attr, 0, 0, timeout));
    if (status.error_code != IBP_OK) {
...
int cmd_split_allocate(char **argv, int argc)
{
    ...
    status = gop_sync_exec_status(ibp_split_alloc_gop(ic, mcap, &caps, size, &attr, 0, 0, timeout));
    if (status.error_code != IBP_OK) {
...
int cmd_merge_allocate(char **argv, int argc)
{
    ...
    if (argc < 3) {
        printf("cmd_merge_allocate: Not enough parameters.  Received %d need 3\n", argc);
        return(0);
    }
    ...
    status = gop_sync_exec_status(ibp_merge_alloc_gop(ic, mcap, ccap, timeout));
    if (status.error_code != IBP_OK) {
...
int cmd_probe(char **argv, int argc)
{
    ...
    if (argc < 2) {
        printf("cmd_probe: Not enough parameters.  Received %d need 2\n", argc);
        return(0);
    }
    ...
    status = gop_sync_exec_status(ibp_probe_gop(ic, cap, &probe, timeout));
    if (status.error_code != IBP_OK) {
        printf("cmd_probe: Error %s(%d)\n", _ibp_error_map[-status.error_code], status.error_code);
        return(0);
    }

    ibp_cap_getstatus(&probe, &rcnt, &wcnt, &csize, &msize, &attr);
    ibp_attributes_get(&attr, &d, &rel, &type);
    duration = d;
    ctime_r(&d, print_time);
...
int cmd_modify_count(int mode, char **argv, int argc)
{
    ...
    status = gop_sync_exec_status(ibp_modify_count_gop(ic, cap, mode, captype, timeout));
    if (status.error_code != IBP_OK) {
...
int cmd_modify_alloc(char **argv, int argc)
{
    ...
    status = gop_sync_exec_status(ibp_modify_alloc_gop(ic, cap, size, duration, rel, timeout));
    if (status.error_code != IBP_OK) {
...
int cmd_version(char **argv, int argc)
{
    ...
    if (argc < 2) {
        printf("cmd_version: Not enough parameters.  Received %d need 3\n", argc);
        return(0);
    }
    ...
    status = gop_sync_exec_status(ibp_version_gop(ic, &depot, buffer, sizeof(buffer), timeout));
    if (status.error_code != IBP_OK) {
        printf("cmd_probe: Error %s(%d)\n", _ibp_error_map[-status.error_code], status.error_code);
        return(0);
    }
...
int cmd_ridlist(char **argv, int argc)
{
    ...
    status = gop_sync_exec_status(ibp_query_resources_gop(ic, &depot, &ridlist, timeout));
    if (status.error_code != IBP_OK) {
        printf("cmd_ridlist: Error %s(%d)\n", _ibp_error_map[-status.error_code], status.error_code);
        return(0);
    }

    printf("Resource list for %s:%s  (%d resources)\n", argv[0], argv[1], ibp_ridlist_size_get(&ridlist));
    for (i=0; i<ibp_ridlist_size_get(&ridlist); i++) {
        ibp_rid2str(ibp_ridlist_element_get(&ridlist, i), srid);
        printf("%s\n", srid);
    }
