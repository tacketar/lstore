...
lio_cache_t *rr_get_handle(lio_cache_t *c)
{
...
int rr_cache_destroy(lio_cache_t *c)
{
    int i;
...
lio_cache_t *round_robin_cache_create(void *arg, data_attr_t *da, int timeout)
{
    ...
    cache->type = CACHE_TYPE_ROUND_ROBIN;
    cache->fn.priv = c;
    if (!cache->fn.priv) {
...
lio_cache_t *round_robin_cache_load(void *arg, tbx_inip_file_t *fd, char *grp, data_attr_t *da, int timeout)
{
    lio_cache_t *c;
    cache_rr_t *cp;
